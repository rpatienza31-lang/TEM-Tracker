...
export type CatalogGeneratorInput = {
  termId: string;
  grades: number[];
  /** grade -> subject ids offered that grade this term */
  subjectsByGrade: Record<number, string[]>;
  /** grade -> subject ids that also get a COT item (subset of subjectsByGrade[grade]) */
  cotByGrade: Record<number, string[]>;
...
function planItems(input: CatalogGeneratorInput): PlannedItem[] {
  ...
  for (const grade of input.grades) {
    ...
    const cotSubjectIds = new Set(input.cotByGrade[grade] ?? []);
    for (const subjectId of subjectIds) {
      for (const week of input.weeks) {
        items.push({ grade, subjectId, weekNumber: week.weekNumber, type: "DLP" });
        if (cotSubjectIds.has(subjectId)) {
          items.push({ grade, subjectId, weekNumber: week.weekNumber, type: "COT" });
        }
      }
    }
  }
